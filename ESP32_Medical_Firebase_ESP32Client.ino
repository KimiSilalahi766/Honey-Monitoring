...
void setup() {
  ...
  Serial.println("\n=== Heart Monitor with Firebase ESP32 Client ===");
  
  // Setup WiFi
  setupWiFi();
  
  // Setup Firebase ESP32 Client
  if (wifiConnected) {
    setupFirebaseESP32Client();
  }
...
void loop() {
  if (digitalRead(BUTTON_PIN) == LOW) {
    ...
    // Kirim ke Firebase dengan ESP32 Client
    if (wifiConnected && firebaseReady) {
      kirimDataKeFirebaseESP32Client();
    } else {
...
void setupFirebaseESP32Client() {
  lcd.setCursor(0, 3);
  lcd.print("Setup Firebase...");
  
  Serial.println("Initializing Firebase ESP32 Client...");
  ...
  if (Firebase.ready()) {
    Serial.println("\n✅ Firebase ESP32 Client Ready!");
    firebaseReady = true;
    lcd.setCursor(0, 3);
    lcd.print("Firebase: OK       ");
    
    // Test write to confirm connection
    if (Firebase.RTDB.setString(&fbdo, "/test/connection", "ESP32_Connected")) {
...
void kirimDataKeFirebaseESP32Client() {
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("Sending to Firebase..");
  
  Serial.println("\n--- Sending to Firebase ESP32 Client ---");
  ...
  // Buat FirebaseJson object
  FirebaseJson json;
  json.set("timestamp", String(timestamp));
  json.set("device_id", "ESP32_Medical_ESP32Client");
  ...
  json.set("waktu_baca", getReadableTime());
  
  // Kirim data utama
  Serial.printf("Set JSON to path: %s\n", dataPath.c_str());
  
  if (Firebase.RTDB.setJSON(&fbdo, dataPath.c_str(), &json)) {
    Serial.println("✅ Firebase ESP32 Client Success!");
    ...
    if (Firebase.RTDB.setJSON(&fbdo, latestPath.c_str(), &json)) {
      ...
      lcd.setCursor(0, 2);
      lcd.print("Latest failed");
    }
    
  } else {
    Serial.println("❌ Firebase ESP32 Client Failed!");
    ...
    lcd.print("Failed!");
    lcd.setCursor(0, 2);
    lcd.print("Code: " + String(fbdo.httpCode()));
  }

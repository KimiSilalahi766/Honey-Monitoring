...
// ===== TIDAK PERLU LIBRARY FIREBASE =====
// Kita pakai SSL fingerprint yang sudah Anda modifikasi di library manual

// Konfigurasi WiFi - GANTI SESUAI JARINGAN ANDA
const char* ssid = "DIR-612";        
const char* password = "passwordwifi";      

// Konfigurasi Firebase - SSL Fingerprint yang sudah Anda modifikasi
...
void setup() {
  ...
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("Medical SSL Direct");
  
  Serial.println("\n=== Heart Monitor - SSL Direct (No Firebase Lib) ===");
  Serial.println("Menggunakan fingerprint: " + String(FIREBASE_FINGERPRINT));
  
  // Setup WiFi
  setupWiFi();
  
  // Setup Firebase SSL (tanpa library)
  if (wifiConnected) {
    setupFirebaseSSLDirect();
  }
...
void setupFirebaseSSLDirect() {
  lcd.setCursor(0, 3);
  lcd.print("Setup Firebase SSL..");
  
  Serial.println("Setup Firebase dengan SSL Fingerprint Direct...");
  ...
  if (client) {
    // Set SSL fingerprint yang sudah Anda modifikasi di library
    client->setFingerprint(FIREBASE_FINGERPRINT);
    
    // Test connection
    Serial.println("Testing SSL connection to Firebase...");
    
    HTTPClient https;
    String testUrl = "https://" + FIREBASE_HOST + "/test_ssl_direct.json";
    
    if (https.begin(*client, testUrl)) {
      https.addHeader("Content-Type", "application/json");
      
      String testData = "\"ssl_direct_test_" + String(millis()) + "\"";
      
      Serial.println("Sending SSL test...");
      int httpResponseCode = https.PUT(testData);
      
      if (httpResponseCode > 0) {
        String response = https.getString();
        Serial.println("✅ SSL Direct Firebase terhubung!");
        ...
        // Hapus test data
        https.DELETE();
        Serial.println("✅ Test data SSL Direct dihapus");
        
      } else {
        firebaseReady = false;
        Serial.println("❌ SSL Direct Firebase gagal");
        ...
        if (httpResponseCode == -1) {
          ...
          Serial.println("💡 Fingerprint might be outdated or network issue");
        } else if (httpResponseCode == 403) {
          Serial.println("🔍 Firebase Rules issue");
          Serial.println("💡 Set Firebase Rules to allow read/write");
        }
      }
      ...
      firebaseReady = false;
      lcd.setCursor(0, 3);
      lcd.print("HTTPS: Init Gagal  ");
    }
    
    delete client;
  } else {
    Serial.println("❌ Unable to create SSL client");
    firebaseReady = false;
  }
...
void kirimDataSSLDirect() {
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("Kirim SSL Direct...");
  
  Serial.println("Mengirim data ke Firebase via SSL Direct...");
  
  WiFiClientSecure* client = new WiFiClientSecure;
  
  if (client) {
    // Set SSL fingerprint yang sudah Anda modifikasi
    client->setFingerprint(FIREBASE_FINGERPRINT);
    ...
    jsonData += "\"device_id\":\"ESP32_SSL_Direct\",";
    ...
    Serial.println("SSL Direct URL: " + url);
    ...
    if (https.begin(*client, url)) {
      ...
      if (httpResponseCode > 0) {
        String response = https.getString();
        Serial.println("✅ Data SSL Direct berhasil dikirim!");
        Serial.println("Response Code: " + String(httpResponseCode));
        Serial.println("Response: " + response);
        
        lcd.setCursor(0, 1);
        lcd.print("SSL Direct: OK!");
        lcd.setCursor(0, 2);
        ...
        if (https.begin(*client, latestUrl)) {
          ...
          if (https.PUT(jsonData) > 0) {
            Serial.println("✅ Latest SSL Direct data updated");
          }
        }
        ...
        lcd.setCursor(0, 1);
        lcd.print("SSL Direct: Gagal");
        lcd.setCursor(0, 2);
        lcd.print("Code: " + String(httpResponseCode));
        
        if (httpResponseCode == -1) {
          Serial.println("🔍 SSL Fingerprint verification failed");
          Serial.println("💡 Check if fingerprint in library matches current certificate");
        }
      }
